...
def run(flags_obj, datasets_override=None, strategy_override=None):
  ...
  malaria = tfds.builder('malaria', data_dir=flags_obj.data_dir)
  if flags_obj.download:
    malaria.download_and_prepare()

  malaria_train = datasets_override or malaria.as_dataset(
      split='train[80%:]',
      decoders={'image': decode_image()},  # pylint: disable=no-value-for-parameter
      as_supervised=True)

  malaria_test = datasets_override or malaria.as_dataset(
      split='train[-20%:]',
      decoders={'image': decode_image()},  # pylint: disable=no-value-for-parameter
      as_supervised=True)

  train_input_dataset = malaria_train.cache().repeat().shuffle(
      buffer_size=50000).batch(flags_obj.batch_size)
  eval_input_dataset = malaria_test.cache().repeat().batch(flags_obj.batch_size)
  ...
  num_train_examples = malaria.info.splits['train'].num_examples
  ...
  num_eval_examples = malaria.info.splits['test'].num_examples
